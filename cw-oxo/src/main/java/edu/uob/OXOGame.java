...
public class OXOGame extends Frame
  ...
  public OXOGame(int width, int height) {
    super("OXO Board");
    OXOModel model = new OXOModel(3, 3, 3);  //初始化board
    model.addPlayer(new OXOPlayer('X'));    //加入新玩家X （玩家定义在OXOPlayer）
    model.addPlayer(new OXOPlayer('O'));    //加入新玩家O
    controller = new OXOController(model);     //controller为OXOController对象 (具体条件在model对象中)
