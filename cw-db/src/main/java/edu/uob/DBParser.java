...
public class DBParser {
    ...
    public DBParser(List<String> commandWord, int pointer, String currentDB, List<DataBase> DBList) throws DBException{
        ...
        DBParser.pointer = pointer;
        DBParser.currentDB = currentDB;
        DBParser.DBList = DBList;
    ...
    public void setIdCounts(List<DBidCount> idCounts){DBParser.idCounts = idCounts;}

    public List<DataBase> getDBList(){return DBList;}

    public List<DBidCount> getIdCounts(){return idCounts;}
    ...
    public void sqlCreate() throws DBException{
        ...
        DBList = DBcmd.DBList;   //reset DBList
        idCounts = DBcmd.idCounts;
    ...
    public void sqlInsert() throws DBException{
        ...
        if (commandWord.get(pointer).equalsIgnoreCase(")")){
            ...
            while(!commandWord.get(pointer).equalsIgnoreCase(")")){
                sqlInsertCmd.valueList.add(commandWord.get(pointer));
                pointer++;
                if ((commandWord.get(pointer).equalsIgnoreCase(","))){
                    pointer++; //comma --> ++
                    if (commandWord.get(pointer).equalsIgnoreCase("）")){
    ...
    public void sqlCondition(DBcmd inputCond) throws DBException{
        ...
        DBcmd.conditions = inputPolish.getConditions();
    }
