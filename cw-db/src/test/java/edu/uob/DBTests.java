...
final class DBTests {
  ...
  void testCondition() {
    ...
    assertTrue(server.handleCommand("SELECT * FROM table1 WHERE (name=='Helen')OR((id<4)AND(age>25));").contains(
            "2\tPeter\t26\tmale\t\r\n"+"3\tTom\t30\tmale\t\r\n"+"5\tHelen\t20\tfemale\t\r\n"));
    assertTrue(server.handleCommand("DELETE FROM table1 WHERE (age<25) AND (gender=='male');").startsWith("[OK]"));
    assertTrue(server.handleCommand("UPDATE table1 SET age = 28 WHERE name == 'Tom';").startsWith("[OK]"));
  }
